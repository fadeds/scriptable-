...
function getSFIcon(code) {
  var night = isNight(), c = parseInt(code, 10)
...
function drawTempBar(minTemp, maxTemp, globalMin, globalMax, barWidth) {
  var ctx = new DrawContext(); ctx.size = new Size(barWidth, 6); ctx.opaque = false
...
async function fetchAll(cfg) {
  ...
  var r = await Promise.all([
    fetchNow(cityId, cfg.apiKey), fetchDaily(cityId, cfg.apiKey),
...
function buildLarge(data) {
  ...
  // ========== 当前天气 ==========
  w.addSpacer(8)
  var cur = w.addStack(); cur.layoutHorizontally(); cur.centerAlignContent()
  var iv = cur.addImage(sfImage(icon, 48)); iv.imageSize = new Size(48, 48); cur.addSpacer(12)
  var ti = cur.addStack(); ti.layoutVertically()
  var tp = ti.addText(now.temp + "°"); tp.font = Font.boldRoundedSystemFont(50); tp.textColor = Color.white()
  tp.lineLimit = 1; tp.minimumScaleFactor = 0.5
  var dr = ti.addStack(); dr.layoutHorizontally(); dr.centerAlignContent()
  var ds = dr.addText(desc); ds.font = Font.regularSystemFont(14); ds.textColor = new Color("ffffff", 0.8)
  dr.addSpacer(8)
  var fl = dr.addText("体感" + now.feelsLike + "°"); fl.font = Font.regularSystemFont(10); fl.textColor = new Color("ffffff", 0.5)
  cur.addSpacer()
  var det = cur.addStack(); det.layoutVertically(); det.topAlignContent()
...
async function main() {
  ...
  if (family) {
    ...
    var map = {
      small: buildSmall, medium: buildMedium, large: buildLarge,
